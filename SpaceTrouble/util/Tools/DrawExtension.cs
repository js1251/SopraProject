...
namespace SpaceTrouble.util.Tools {
    internal static class DrawExtension {
        ...
        private static readonly Color[] sColors = {Color.Red, Color.Orange, Color.Yellow, Color.Green, Color.Blue, Color.Indigo, Color.Purple};
        ...
        public static void DrawPolygon(this SpriteBatch spriteBatch, Vector2[] vertex, int count, Color color, float thickness = 1f) {
            if (count > 0) {
                for (var i = 0; i < count - 1; i++) {
                    DrawLine(spriteBatch, vertex[i], vertex[i + 1], color, thickness);
                }
        ...
        public static void DrawCircle(this SpriteBatch spriteBatch, Vector2 center, float radius, Color color, float thickness = 1f, int segments = 16) {
